...
const JobAnalysis = () => {
    ...
    const handleAnalyze = async () => {
        ...
        try {
            ...
            console.error(err);
            setModalError(err.response?.data?.message || "정보를 불러오는데 실패했습니다.");
            setShowErrorModal(true);
        } finally {
            setLoading(false);
        }
    ...
    const handleRegister = async () => {
        ...
        if (!result.jobPostingId) {
            alert("Internal Error: JobPosting ID is missing in result: " + JSON.stringify(result));
            return;
        }
        ...
        try {
            ...
            if (err.response?.status === 409) {
                ...
                setModalError(err.response?.data?.message || '등록 저장에 실패했습니다.');
                setShowErrorModal(true);
            }
        } finally {
        ...
        <div className="bg-white min-h-screen pb-safe relative flex flex-col">
            ...
            <div className="p-5 flex-1 relative">
                ...
                <div className="relative">
                    <div className={`bg-gray-200 rounded-xl p-1 flex items-center pr-2 ${error ? 'border border-red-500' : ''}`}>
                        <input
                            type="text"
                    ...
                    <div className="mt-8 pb-24 animate-fade-in-up">
                        ...
                        <div className="border border-red-400 rounded-2xl p-5 relative">
                            ...
                            <div className="grid grid-cols-2 gap-y-6 gap-x-4 mb-6">
                                {/* Company */}
                                <div>
                                    <div className="text-xs text-gray-900 font-bold mb-1">기업명</div>
                                ...
                                {/* Job Title */}
                                <div>
                                    <div className="text-xs text-gray-900 font-bold mb-1">직무 명</div>
                                ...
                                <div className="col-span-1">
                                    <div className="text-xs text-gray-900 font-bold mb-1">주요 업무</div>
                                    <div className="bg-gray-200 rounded-lg px-3 py-2 text-xs text-gray-700 font-medium h-full flex items-center">
                                        <span className="line-clamp-1">
                                            {result.mainTasks && result.mainTasks.length > 0 ? result.mainTasks.join(', ') : '-'}
                                        </span>
                                    </div>
                                </div>
                                {/* Tech Stack */}
                                <div className="col-span-1">
                                    <div className="text-xs text-gray-900 font-bold mb-1">필요기술스택</div>
                                    <div className="bg-gray-200 rounded-lg px-3 py-2 text-xs text-gray-700 font-medium h-full flex items-center">
                                        <span className="line-clamp-1">
                                            {result.skills && result.skills.length > 0 ? result.skills.join(', ') : '-'}
                                        </span>
                                    </div>
                                </div>
                                ...
                                {/* Status */}
                                <div>
                                    <div className="text-xs text-gray-900 font-bold mb-1">모집상태</div>
                                ...
                                {/* Period */}
                                <div>
                                    <div className="text-xs text-gray-900 font-bold mb-1">모집 기간</div>
                                    <div className="bg-gray-200 rounded-lg px-3 py-2 text-xs text-gray-700 font-medium flex items-center h-full">
                                        {(result.startDate || 'YYYY.MM.DD').replaceAll('-', '.')}~{(result.endDate || 'YYYY.MM.DD').replaceAll('-', '.')}
                                    </div>
                                </div>
                            </div>
                            ...
                            {/* AI Summary */}
                            <section>
                                <div className="mb-2 text-xs font-bold text-gray-900">AI공고 요약</div>
                ...
                <div className="fixed inset-0 bg-black/50 z-[200] flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl w-full max-w-[300px] p-6 text-center shadow-xl animate-scale-in">
                        <p className="text-gray-800 mb-6 font-semibold whitespace-pre-wrap text-sm leading-relaxed">
                            작성 중인 내용이 저장되지 않습니다.<br />취소하시겠습니까?
                        </p>
